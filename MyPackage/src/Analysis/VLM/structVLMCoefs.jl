...
_coef_dist(vd::Vector{Vector{Float64}}, denom::Float64) = [d ./ denom for d in vd]
...
function _calc_coefs(
    ...
    return (
        _coef_vec(point.L, qS), _coef_dist(point.L_dist, qS), point.L_total / qS,
        _coef_vec(point.D, qS), _coef_dist(point.D_dist, qS), point.D_total / qS,
        _coef_vec(point.D_trefftz, qS), _coef_dist(point.D_trefftz_dist, qS),
            point.D_trefftz_total / qS,
        _coef_vec(point.FY, qS), _coef_dist(point.FY_dist, qS), point.FY_total / qS,
        _coef_vec(point.M, qSc), _coef_dist(point.M_dist, qSc), point.M_total / qSc,
        _coef_vec(point.Ml, qSb), _coef_dist(point.Ml_dist, qSb), point.Ml_total / qSb,
        _coef_vec(point.N, qSb), _coef_dist(point.N_dist, qSb), point.N_total / qSb,
    )
end
...
println("CL = ", coefs.CL_total, ", CD = ", coefs.CD_total, ", CM = ", coefs.CM_total)
```
"""
...
function VLMCoefficients(setup::VLMSetup, alpha::Float64, beta::Float64)::VLMCoefficients
    point = get(setup.loads.points, (alpha, beta), nothing)
    point === nothing &&
        throw(ArgumentError("no solved point at (alpha, beta) = ($alpha, $beta) -- call VLMSolver! first"))
...
plot(polar.alpha, polar.CL_total, xlabel="alpha (deg)", ylabel="CL", label="Total Lift")
```
"""
function VLMCoefficientsSlice(
    ...
    coefs = (
        CL=slice.L ./ qS, CL_total=slice.L_total ./ qS,
        CD=slice.D ./ qS, CD_total=slice.D_total ./ qS,
        CD_trefftz=slice.D_trefftz ./ qS, CD_trefftz_total=slice.D_trefftz_total ./ qS,
        CY=slice.FY ./ qS, CY_total=slice.FY_total ./ qS,
        CM=slice.M ./ qSc, Cm_total=slice.M_total ./ qSc,
        CMl=slice.Ml ./ qSb, Cl_total=slice.Ml_total ./ qSb,
        CN=slice.N ./ qSb, Cn_total=slice.N_total ./ qSb,
    )

    return alpha === nothing ? merge((alpha=slice.alpha,), coefs) : merge((beta=slice.beta,), coefs)
end