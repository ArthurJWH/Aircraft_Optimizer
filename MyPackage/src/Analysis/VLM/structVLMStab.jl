...
function VLMStabilityDerivatives(
    ...
    d_alpha = Tuple(
        (
            getfield(c_ap, Symbol(f, :_total)) -
            getfield(c_am, Symbol(f, :_total))
        ) / (2 * dalpha) for f in _VLM_STABILITY_FIELDS
    )
    d_beta = Tuple(
        (
            getfield(c_bp, Symbol(f, :_total)) -
            getfield(c_bm, Symbol(f, :_total))
        ) / (2 * dbeta) for f in _VLM_STABILITY_FIELDS
    )

    return VLMStabilityDerivatives(
        alpha, beta, dalpha, dbeta, d_alpha..., d_beta...
    )
end
...
function _solve_point_fast(
    ...
    rings, n_panels, n_surfaces, surfaces = geometry.rings,
    geometry.n_panels, geometry.n_surfaces,
    geometry.surfaces
    CG = geometry.CG
...
function VLMStabilityDerivativesFast(
    ...
    pt_am = _solve_point_fast(
        F, AIC_vx, AIC_vy, AIC_vz, setup, alpha - dalpha, beta
    )
    pt_ap = _solve_point_fast(
        F, AIC_vx, AIC_vy, AIC_vz, setup, alpha + dalpha, beta
    )
    pt_bm = _solve_point_fast(
        F, AIC_vx, AIC_vy, AIC_vz, setup, alpha, beta - dbeta
    )
    pt_bp = _solve_point_fast(
        F, AIC_vx, AIC_vy, AIC_vz, setup, alpha, beta + dbeta
    )
